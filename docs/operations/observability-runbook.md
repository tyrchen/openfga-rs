...
| `openfga.transport.request.duration` | `openfga_transport_request_duration_seconds` | `endpoint_class` | admitted lifetime histogram |
...
| `openfga.cache.controller.lag` | `openfga_cache_controller_lag_milliseconds` | none | maximum observed cursor lag |
...
| `OpenFgaCacheControllerLagging` | Cache eligibility is at risk; check changelog storage latency/backlog and do not raise `maximumLagMs` during diagnosis |
...
After recovery, verify readiness, zero/declining overload, controller lag below policy, returned work
