...
| Cache controller lag/failure/overflow | Mutable decision and tuple caches flush and remain disabled until a healthy caught-up poll; readiness fails | Inspect changelog latency/order, tracked-store/channel bounds, and controller logs; preserve the lag ceiling and authoritative fallback |
| Transport concurrency exhausted | Authorized requests receive bounded overload responses without waiting in an application queue | Shed upstream traffic and compare pool wait, CPU, and in-flight metrics before applying the capacity runbook |
| PostgreSQL work permits exhausted | Storage work waits only until request cancellation/deadline | Inspect pool idle/open state, query latency, locks, and nested evaluator fan-out; retain per-root control-plane headroom |
