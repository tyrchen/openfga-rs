...
| TLS/secrets | stable `rustls 0.23.43` and `axum-server 0.8.0` with `aws-lc-rs`; `secrecy 0.10.3` | Adopt stable rustls only—not the 0.24 development release—and redacting secret types; use the same certificate/key material for the pre-bound Axum and Tonic listeners. [rustls](https://github.com/rustls/rustls), [secrecy](https://github.com/iqlusioninc/crates/tree/main/secrecy). |
