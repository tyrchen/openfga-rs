...
| Validation | `prost-validate-types 0.2.9`, `prost-validate 0.2.9`, `regex 1.13.1`, plus domain constructors | Adopt a project-owned exhaustive interpreter for the PGV annotations pinned in the reflected OpenFGA descriptor set, then convert into private-field domain newtypes. The interpreter evaluates every original rule and nested collection item in generated `ValidateAll` order; it does not mutate requests, truncate diagnostics, or depend on a fail-fast registry. The direct crates are Apache-2.0/MIT, pure Rust, and current as checked on 2026-08-05. Generated derive validation was rejected because it is fail-fast and its `ignore_empty` control flow can skip later fields. [prost-validate repository](https://github.com/linka-cloud/prost-validate), [regex repository](https://github.com/rust-lang/regex). |
| Telemetry | `tracing 0.1.44`, `tracing-subscriber 0.3.23`, `tracing-opentelemetry 0.33.0`, `opentelemetry 0.32.0`, `opentelemetry-otlp 0.32.0` | Adopt structured local logs plus optional OTLP trace export with bounded flush; metrics may use the OTel metrics SDK or Prometheus exporter through one facade. [Official OTel Rust repository](https://github.com/open-telemetry/opentelemetry-rust). |
| TLS/secrets | stable `rustls 0.23.43`, `tokio-rustls 0.26.4`, and `axum-server 0.8.0` with `aws-lc-rs`; `secrecy 0.10.3` | Adopt stable rustls only—not the 0.24 development release—and redacting secret types; `tokio-rustls` provides the async acceptor needed to atomically share reloaded identity across pre-bound Axum and Tonic listeners. [rustls](https://github.com/rustls/rustls), [tokio-rustls](https://docs.rs/tokio-rustls/0.26.4/tokio_rustls/), [secrecy](https://github.com/iqlusioninc/crates/tree/main/secrecy). |
