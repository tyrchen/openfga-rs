...
The passing corpus contains 11 cases:
...
- ListUsers direct, wildcard, recursive userset, intersection, and difference;
...
The runtime disconnect test exercises Check and StreamedListObjects through real HTTP and gRPC listeners. It waits until model storage work is active, disconnects or aborts the client, then proves the request cancellation token is cancelled, active work returns to zero, listener tasks drain, and the memory actor can be uniquely owned and stopped. This test found and fixed the pre-stream ownership gap: streamed model resolution is now guarded until cancellation responsibility transfers to the returned stream.
...
The PostgreSQL plan test is intentionally ignored without an isolated database. No `OPENFGA_POSTGRES_TEST_URL` or `POSTGRES_TEST_URL` was available for this local run, so no live PostgreSQL result is claimed here. Run it with:
...
The first three commands and the full workspace gates passed on the final Task 3.5 diff. The PostgreSQL test was compiled but remained ignored because its isolated-database environment variable was absent, as disclosed above.
